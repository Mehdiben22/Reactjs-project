...
const ContactDetail = ({updateContact , updateImage }) => {
    ...
    const [contact , setContact] = useState({
      ...
      //give access to the data and the url and pulling out the id 
      const {id} = useParams();
      ...
      const selectImage = () => {
       ...
       const PhotoUp = async(file) => {
        try{
            ...
            // attach the user id too
            formData.append('id', id);
             await updateImage(formData);
             //every time we update the photo we gonna update the url and the updated time that the url was updated it
  ...
  return (
    <>
    <Link to={'/'} className='link'><i className='bi bi-arrow-left'></i>
        Back to List
    </Link>
 ...
 <input type='file' ref={inputRef} onChange={(event) => PhotoUp(event.target.files[0])} name='file' accept='image/*' />
 </form>
    </>
  )
}
